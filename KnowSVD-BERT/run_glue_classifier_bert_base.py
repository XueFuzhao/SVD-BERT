...
def main():
    ...
    parser.add_argument('--use_deepscale',
                        default=False,
                        action='store_true',
                        help="Whether to use 16-bit float precision instead of 32-bit")
    ...
    #MoE Options
    parser.add_argument('--expert_dropout',
                        type=float, default=0.1,
                        help="Dropout Value of Expert FFN")
    parser.add_argument(
        '--use_moe',
        action='store_true',
        default=False,
    ...
    if args.progressive_layer_drop:
        ...
        from pytorch_pretrained_bert.modeling import BertForSequenceClassification, BertConfig, BertLayer
    ...
    model = BertForSequenceClassification.from_pretrained(pretrained_model_name='bert-base-uncased',state_dict=args.model_file ,args=args,num_labels=num_labels)
    ...
    if args.not_update_moe:
        out_para = []
        for n,p in param_optimizer:
            if "moe" not in n:
                out_para.append((n,p))
        param_optimizer = out_para
    ...
    def eval():
        ...
        if torch.distributed.get_rank()==0:
            print("Eval Accuracy",result["acc"])
    ...
    #def train():
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if args.use_deepscale and args.local_rank != -1:
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if (step + 1) % (args.eval_step*args.gradient_accumulation_steps) == 0:
                        if torch.distributed.get_rank() == 0:
                            print("Training Loss:", tr_loss/nb_tr_steps*args.gradient_accumulation_steps)
                        eval()
                        model.train()
                    optimizer.step()
                    optimizer.zero_grad()
