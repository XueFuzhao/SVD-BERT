...
#from pytorch_pretrained_bert.modeling import BertForPreTrainingPreLN
...
class BertMultiTask_Continue_training:
    def __init__(self, args):
        self.config = args.config
        #self.args = args
        if not args.use_pretrain:
        ...
        # Use pretrained bert weights
            ...
            #bert_config = json.load(open(args.pretrained_BERT_config, 'r', encoding='utf-8'))
            from pytorch_pretrained_bert.modeling import BertForPreTrainingPreLN
            #print(self.config['bert_model_file'])
            #print(self.config['bert_model_file'][0])
            self.network = BertForPreTrainingPreLN.from_pretrained(
                args,
                args.model_name,
                cache_dir=PYTORCH_PRETRAINED_BERT_CACHE /
                'distributed_{}'.format(args.local_rank))
            #bert_config = self.config['bert_model_file']
            #bert_config = self.bert_encoder.config
        self.device = None
