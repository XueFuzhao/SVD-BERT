...
class BertFFN(nn.Module):
    ...
    def forward(self, input_tensor):
        intermediate_output = self.intermediate(input_tensor)
...
class BertLayer(nn.Module):
    def __init__(self, config, args, use_moe_here=None):
        ...
        if use_moe_here:
            self.FFN_moe = deepspeed.moe.layer.MoE(
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        if self.use_moe_here:
            layer_output, loss, _ = self.FFN_moe(attention_output)
            #intermediate_output = self.intermediate(attention_output)
            #layer_output = self.output(intermediate_output, attention_output)
            layer_output = self.FFN(attention_output) + layer_output
...
class BertEncoder(nn.Module):
    ...
    def forward(self,
        ...
        if checkpoint_activations:
            ...
            if self.use_moe:
                ...
                for i, layer_module in enumerate(self.layer):
                    ...
                    tmp_load_balance_loss += tmp_load_balance_loss
                    if output_all_encoded_layers:
                        all_encoder_layers.append(hidden_states)
            else:
                for i, layer_module in enumerate(self.layer):
        ...
        if not output_all_encoded_layers or checkpoint_activations:
            ...
            all_encoder_layers.append(hidden_states)
        if self.use_moe:
            return all_encoder_layers,tmp_load_balance_loss
        else:
            return all_encoder_layers
...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(self, sequence_output, pooled_output, masked_token_indexes):
...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls,
        ...
        if num_labels is not None:
            model = cls(args, config, num_labels, *inputs, **kwargs)
        else:
            model = cls(args, config, *inputs, **kwargs)

        #print("=============================================")
        #print(state_dict)
        state_dict_flag = state_dict
        if state_dict is None:
            print("Loading from Google Checkpoint")
            weights_path = os.path.join(serialization_dir, WEIGHTS_NAME)
            state_dict = torch.load(weights_path)
        else:
            if args.use_moe:
                expert_dir = state_dict[:-26]
                print("expert dir:")
                print(expert_dir)
                #expert_path = []
                expert_dict = {}
                for expert_id in range(args.num_experts):
                    #expert_path.append(expert_dir+"")
                    expert_dict.update(torch.load(expert_dir+"expert_"+str(expert_id)+"_mp_rank_00_model_states.pt",map_location='cuda:0'))
            
                state_dict = torch.load(state_dict,map_location='cuda:0')['module']
            if args.use_moe:
                state_dict.update(expert_dict) 
            for key in state_dict.keys():
                print(key)
            #weights_path = os.path.join(s, WEIGHTS_NAME)
        

        if state_dict_flag is None:
            old_keys = []
            new_keys = []
            for key in state_dict.keys():
                new_key = None
            ...
            for key in state_dict.keys():
                new_key = None
                if 'intermediate' in key and 'attention' not in key:
            ...
            for key in state_dict.keys():
                ...
                if 'FFN.output.LayerNorm' in key:
                    new_key = key.replace('FFN.output.LayerNorm', 'LayerNorm')
                #    new_key = key.replace('.FFN.output.LayerNorm', '.LayerNorm')
                if new_key:
                    old_keys.append(key)
                    new_keys.append(new_key)
        ...
        if args.use_moe and state_dict_flag is None:
            ...
            for i in range(config.num_hidden_layers-args.num_moe_layers-args.post_moe_layers,  config.num_hidden_layers -args.post_moe_layers):
                for FFN_type in ["intermediate","output"]:
                    for para in ["weight","bias"]:
                        ...
                        for expert_id in range(args.num_experts):
                            #bert.encoder.layer.10.FFN.deepspeed_moe.experts.deepspeed_experts.2.intermediate.dense.weight
                            new_key = 'bert.encoder.layer.'+str(i)+'.FFN_moe.deepspeed_moe.experts.deepspeed_experts.'
                            new_key = new_key +str(expert_id)+'.'+FFN_type+'.dense.'+para
                            if args.use_svd:
                                if para=="weight":
                                    def SVD_weight(v):
                                        u_, s, v_ = torch.svd(v,some=True)
                                        s_=torch.nn.functional.dropout(s,p=0.875)
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self,
        ...
        if self.use_moe:
            encoded_layers , load_balance_loss = self.encoder(
                embedding_output,
        ...
        if self.use_moe:
            return encoded_layers, pooled_output, load_balance_loss
        else:
...
class BertForPreTrainingPreLN(PreTrainedBertModel):
    """BERT model with pre-training heads.
    This module comprises the BERT model followed by the two pre-training heads:
        - the masked language modeling head, and
        - the next sentence classification head.
    ...
    def forward(self, batch, log=True):
        ...
        if self.use_moe:
            sequence_output, pooled_output, load_balance_loss = self.bert(
                input_ids,
                token_type_ids,
                attention_mask,
                output_all_encoded_layers=False)
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            next_sentence_loss = loss_fct(seq_relationship_score.view(-1, 2),
                                          next_sentence_label.view(-1))
            if self.use_moe:
                total_loss = masked_lm_loss + next_sentence_loss + 0.01 * load_balance_loss
            else:
                total_loss = masked_lm_loss + next_sentence_loss
            return total_loss
        else:
...
class BertForSequenceClassification(PreTrainedBertModel):
    ...
    def forward(self,
                ...
                labels=None):
        if self.use_moe:
            _, pooled_output, load_balance_loss = self.bert(input_ids,
                                         token_type_ids,
                                         attention_mask,
                                         output_all_encoded_layers=False)
        else:
