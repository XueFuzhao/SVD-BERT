...
from turing.models import BertMultiTask_Continue_training
...
from pytorch_pretrained_bert.optimization import BertAdam, warmup_linear,warmup_linear_decay_linear, warmup_linear_decay_exp, warmup_exp_decay_exp, warmup_exp_decay_poly
from utils import get_argument_parser, is_time_to_exit

from bing_bert_dataset_provider import BingBertDatasetProvider
from nvidia_bert_dataset_provider import NvidiaBertDatasetProvider,NvidiaBertTestDatasetProvider
...
def pretrain_validation(args, index, model, current_data_sample_count):
    ...
    '''dataset = PreTrainingDataset(
        args.tokenizer,
        args.validation_data_path_prefix, args.logger,
        args.max_seq_length, index, PretrainDataType.VALIDATION,
        args.max_predictions_per_seq)
    data_batches = get_dataloader(args, dataset, eval_set=True)
    '''
    pretrain_val_dataset_provider = NvidiaBertTestDatasetProvider(args)
    #pretrain_val_dataset_provider = NvidiaBertDatasetProvider(args)
    #index = 252
    dataset_iterator, total_length = pretrain_val_dataset_provider.get_shard(index)
    ...
    for _, batch_index in enumerate(tqdm(dataset_iterator, smoothing=1)):
        try:
            ...
            batch = pretrain_val_dataset_provider.get_batch(batch_index)
            batch = tuple(t.to(args.device) for t in batch)  # Move to GPU

            # Prefetch training data
            pretrain_val_dataset_provider.prefetch_batch()
            ...
            eval_loss += tmp_eval_loss.mean().detach()
            nb_eval_steps += 1
    ...
    dist.reduce(eval_loss, 0)
    
    if dist.get_rank() == 0:
        eval_loss = eval_loss.item() / (nb_eval_steps*dist.get_world_size())
    logger.info(f"Validation Loss for epoch {index + 1} is: {eval_loss}")
    pretrain_val_dataset_provider.release_shard(index)
...
def train(args,
    ...
    #if index == 0:
    #    pretrain_validation(args, index, model, current_data_sample_count)
    ...
    for _, batch_index in enumerate(tqdm(dataset_iterator, smoothing=1)):
        try:
            ...
            if model.network.is_gradient_accumulation_boundary():
                ...
                #if (global_step + 1) % args.print_steps == 0:
                #    pretrain_validation(args, index, model,current_data_sample_count)
...
def update_learning_rate(args, config, current_global_step, optimizer):
    ...
    if args.lr_schedule == "EE":
        ...
        lr_this_step = config["training"]["learning_rate"]* warmup_linear_decay_linear(
                 global_step_for_lr, config["training"]["total_training_steps"],config["training"]["warmup_proportion"])
    else:
        lr_this_step = config["training"][
...
def prepare_optimizer_parameters(args, model):
    ...
    if args.moe_only:
        print("Original parameters in optimizer:")
        
        #print(param_optimizer)
        for n in param_optimizer:
            print(n[0])

        untrained_prefix = []
        untrained_prefix.append("bert.embeddings.")
        if "base" in args.model_name:
            num_hidden_layers = 12
        elif "large" in args.model_name:
            num_hidden_layers = 24
        for i in range(num_hidden_layers-args.num_moe_layers-args.post_moe_layers):
            untrained_prefix.append("bert.encoder.layer."+str(i)+".")
        print(model.network.parameters())
        for n,p in model.network.named_parameters():
            #untrained_prefix = []
            
            for name in untrained_prefix:
                if name in n:
                    p.requires_grad = False
        for name in untrained_prefix:
            param_optimizer = [n for n in param_optimizer if name not in n[0]]

        print("Updated parameters in optimizer:")
        #print(param_optimizer)
        for n in param_optimizer:
            print(n[0])
...
def prepare_model_optimizer(args):
    ...
    model = BertMultiTask_Continue_training(args)
    #model = BertForPreTraining(args.config['bert_model_config'])
    ...
    #print("=============================================")
    #print(args.train_micro_batch_size_per_gpu)
...
def run(args, model, optimizer, start_epoch):
    ...
    if args.use_nvidia_dataset:
        #pretrain_dataset_provider = NvidiaBertTestDatasetProvider(args)
