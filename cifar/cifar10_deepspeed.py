...
def add_argument():
    ...
    parser.add_argument(
        '--model_path',
        default='/home/users/nus/e0792473/scratch/DeepSpeedExamples/cifar/model/dense_cnn',
        type=str
            )
...
pretrained_dict = torch.load( args.dense_model_path+'cnn_model_weights.pt')
...
for k,v in pretrained_dict.items():
    if k in model_dict and not "fc3" in k:
        model_dict[k]=v
        #print(k)
        if k=='fc3.weight':
            for k_m, v_m in model_dict.items():
                if 'weight' in k_m and 'expert' in k_m:
            ...
            for k_m, v_m in model_dict.items():
                if 'bias' in k_m and 'expert' in k_m:
                    model_dict[k_m]=v
            ...
            with torch.no_grad():
                for data in testloader:
                    ...
                    if fp16:
                        images = images.half()
                    if args.moe:
                        outputs,_ = net(images.to(model_engine.local_rank))
                    else:
...
torch.save(net.state_dict(), args.model_path+'cnn_model_weights.pt')
